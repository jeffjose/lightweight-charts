import { Callback, Callback2, ISubscription, ISubscription2 } from './isubscription';
...
interface Listener2<T1, T2, T3> {
	callback: Callback2<T1, T2, T3>;
	linkedObject?: unknown;
	singleshot: boolean;
}
...
// eslint-disable-next-line @typescript-eslint/no-invalid-void-type
export class Delegate2<T1 = void, T2 = void, T3 = void> implements ISubscription2<T1, T2, T3> {
	private _listeners: Listener2<T1, T2, T3>[] = [];
    ...
    singleshot?: boolean
		...
		const listener: Listener2<T1, T2, T3> = {
			callback,
			linkedObject,
			singleshot: singleshot === true,
		};
	...
	public unsubscribe(callback: Callback2<T1, T2, T3>): void {
		const index = this._listeners.findIndex((listener: Listener2<T1, T2, T3>) => callback === listener.callback);
		if (index > -1) {
			this._listeners.splice(index, 1);
		}
	}
	...
	public unsubscribeAll(linkedObject: unknown): void {
		this._listeners = this._listeners.filter((listener: Listener2<T1, T2, T3>) => listener.linkedObject !== linkedObject);
	}

	public fire(param1: T1, param2: T2, param3: T3): void {
		const listenersSnapshot = [...this._listeners];
		this._listeners = this._listeners.filter((listener: Listener2<T1, T2, T3>) => !listener.singleshot);
		listenersSnapshot.forEach((listener: Listener2<T1, T2, T3>) => listener.callback(param1, param2, param3));
	}
